...
Begin
{
    ...
    $LogFile = Join-Path -Path $LogFolder -ChildPath ($Date + " - TestEdgeSync.log")

    $ErrorFound = $false
}
Process
{
    Write-Log -Message "Starting Process Block of TestEdgeSync.ps1" -Level "Info" -Path $LogFile
    ...
    Write-Log -Message "Ending Process Block of TestEdgeSync.ps1" -Level "Info" -Path $LogFile
}
End
{
